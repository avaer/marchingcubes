...
//-----------------------------------------------------------------------------
  ...
  _x_verts(size_x * size_y * size_z, -1),
  _y_verts(size_x * size_y * size_z, -1),
  _z_verts(size_x * size_y * size_z, -1)
{}
//_____________________________________________________________________________
...
//-----------------------------------------------------------------------------
{
  ...
  for( _i = 0 ; _i < _size_x-1 ; _i++ )
  {
    ...
    for( int p = 0 ; p < 8 ; ++p )
    {
			cube[p] = get_data(ivec3(_i+((p^(p>>1))&1), _j+((p>>1)&1), _k+((p>>2)&1))) - iso ;
	...
	for(_k=0; _k < _size_z; ++_k) {
		for(_j=0; _j < _size_y; ++_j) {
			for(_i=0; _i < _size_x; ++_i) {
				...
				if( cube[0] < 0 )
				{
					if( cube[1] > 0 ) set_x_vert( add_vertex(grid_coord, ivec3(1, 0, 0), 1, cube), _i,_j,_k ) ;
					if( cube[3] > 0 ) set_y_vert( add_vertex(grid_coord, ivec3(0, 1, 0), 3, cube), _i,_j,_k ) ;
					if( cube[4] > 0 ) set_z_vert( add_vertex(grid_coord, ivec3(0, 0, 1), 4, cube), _i,_j,_k ) ;
				}
				else
				{
					if( cube[1] < 0 ) set_x_vert( add_vertex(grid_coord, ivec3(1, 0, 0), 1, cube), _i,_j,_k ) ;
...
//-----------------------------------------------------------------------------
{
  if(i > 0) {
		if(i < _size_x - 1 ) {
			return ( get_data(ivec3(i+1, j, k)) - get_data(ivec3(i-1, j, k ))) / 2 ;
		}
		else {
      return get_data(ivec3(i, j, k)) - get_data(ivec3(i-1, j, k)) ;
		}
  }
	else {
    return get_data(ivec3(i+1, j, k)) - get_data(ivec3(i, j, k)) ;
	}
}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
{
  if(j > 0) {
		if (j < _size_y - 1) {
			return ( get_data(ivec3(i, j+1, k)) - get_data(ivec3(i, j-1, k)) ) / 2 ;
		}
		else {
      return get_data(ivec3(i, j, k)) - get_data(ivec3(i, j-1, k));
		}
  }
	else {
    return get_data(ivec3(i, j+1, k)) - get_data(ivec3(i, j, k));
	}
}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
{
  if(k > 0) {
		if(k < _size_z - 1) {
      return ( get_data(ivec3(i, j, k+1)) - get_data(ivec3(i, j, k-1)) ) / 2 ;
		}
		else {
      return get_data(ivec3(i, j, k)) - get_data(ivec3(i, j, k-1)) ;
		}
  }
	else {
    return get_data(ivec3(i, j, k+1)) - get_data(ivec3(i, j, k)) ;
	}
}
//_____________________________________________________________________________
...
int MarchingCubes::add_vertex(const ivec3 &grid_coord, const ivec3 &dir, int corner, float *cube) {
	auto u = cube[0] / (cube[0] - cube[corner]);
	auto pos = vec3(grid_coord) + vec3(dir) * u;
	...
	auto n = normalize(vec3(nx, ny, nz));
